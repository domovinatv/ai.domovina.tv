...
>
> **D-bis: odvojen GCP PROJEKT, ne samo odvojen client (odluka 2026-06-01).** GCP ima **jedan consent screen po projektu** (branding + publishing status + registrirani scopeovi + verifikacija su per-projekt; više clienata dijeli isti consent screen). Zato `youtube.readonly` (sensitive scope, traži punu Google verifikaciju: domain ownership, home-page purpose, demo video, scope justification, review tjednima) živi u **zasebnom GCP projektu `domovina-creators`**, dok generički login (`openid/email/profile`, non-sensitive) ostaje u **`domovina-auth`**. Ista `domovina.ai` aplikacija se autentificira protiv **dva GCP projekta** — Google to ocjenjuje neovisno i tretira kao normalno (isti vlasnik, konzistentan branding). **Razlozi:** (1) blast-radius — login je auth za cijelu app i ne smije ovisiti o projektu koji se može suspendirati zbog YouTube politike; (2) login consent screen ostaje non-sensitive (glatko u produkciju); (3) zasebna YouTube Data API kvota (per-projekt). **Uvjeti:** konzistentan branding ("DOMOVINA.ai") i verificirana domena `domovina.ai` u **oba** projekta. Backend: `YOUTUBE_CLAIM_GOOGLE_CLIENT_ID/_SECRET` pokazuju na client iz `domovina-creators`; `GOTRUE_EXTERNAL_GOOGLE_*` (login) ostaju u `domovina-auth`. Redirect URIs novog clienta: `http://localhost:5173/youtube-claim/callback` + `https://domovina.ai/youtube-claim/callback`; **YouTube Data API v3** enable-an u `domovina-creators`.
