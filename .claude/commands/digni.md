...
3. **Pokreni**, s promptom kao JEDNIM argumentom u jednostrukim navodnicima:

   ```bash
   ./scripts/tim-open.sh 'Predaj timu … Plan: docs/plans/….md'
   ```
...
4. **Javi ishod** u dvije-tri rečenice: je li tim nov ili se prozor attachao na
...
- **Ako session već postoji, skripta NE šalje prompt** — samo otvori prozor
  koji se attacha, i ispiše komandu za ručno slanje. Tako je namjerno: u
  planner panel korisnik tipka i poruka bi mu upala usred prompta. Ne
  zaobilazi to s `--force` osim ako je korisnik izričito rekao da je panel
  prazan.
