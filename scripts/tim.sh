...
tmux set -t "$SESSION" pane-border-format ' #{pane_id} #{@role} '
tmux set -t "$SESSION" mouse on
tmux set -w -t "$SESSION" automatic-rename off
tmux rename-window -t "$SESSION" tim
...
tmux set -t "$SESSION" status-right-length 140
tmux set -t "$SESSION" status-right "#(cat '$PWD/.tim/status.line' 2>/dev/null) "
