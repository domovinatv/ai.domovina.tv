...
REPO_RULES='Repo je Flutter web/mobile app (DOMOVINA.ai). CLAUDE.md u rootu je obavezno štivo — poštuj sve Rule sekcije (i18n kroz app_hr.arb pa app_en.arb, nikad SharedPreferences na webu, theme tokeni umjesto konstanti, padding unutar scrollabla). Kod aplikacije je u paketu packages/podcast_core (testovi se pokreću iz tog direktorija); korijen je samo DOMOVINA ljuska. Verifikacija: `flutter analyze` mora biti čist. NAPOMENA: test/widget_test.dart (HttpClient smoke) i home_feed_test (datum-ovisan) padaju i na čistom mainu — to NISU regresije.'
